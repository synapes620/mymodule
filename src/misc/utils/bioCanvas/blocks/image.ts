...
export const IMAGE_CROP_FITS = ['cover', 'contain'] as const;
...
export type ImageCropData = z.infer<typeof imageCropSchema>;
...
export function normalizeImageCrop(raw: unknown): ImageCropData {
...
export const imageBlockDescriptor = {
  ...
  createDefault: (): ImageBlockData => ({ crop: { ...DEFAULT_IMAGE_CROP } }),
  toPlainText: (data: ImageBlockData): string => (data.alt?.trim() ? `[image: ${data.alt}]` : '[image]'),
};
